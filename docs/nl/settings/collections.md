...
> [!CAUTION]
> Het maken van collecties moet worden beschouwd als een functie voor geavanceerde gebruikers (serieus, dit is een waarschuwing). Als je jezelf geen geavanceerde gebruiker vindt, wordt aangeraden om in plaats daarvan een collectie te kopiëren van [Nuvio's community-collecties](https://nuvio.tv/community-collections).

---

### De structuur van een collectie begrijpen

Nuvio-collecties kunnen verwarrend zijn, maar in de basis zijn ze gewoon een bestandssysteem: mappen met daarin submappen, die bestanden (catalogi) bevatten. Het onderstaande diagram illustreert deze relatie.
...
[Terug naar boven](#collecties)

### Voordat je begint

Je hebt twee dingen nodig om een collectie te maken:

1. **Een metadata-add-on met je catalogi** die al is ingesteld en de catalogi bevat die je wilt opnemen. Deze gids gaat ervan uit dat dit is gebeurd.
2. **Afbeeldingen of GIF's voor je collectie-covers**, afkomstig van internet of van [Nuvio's community covers](https://nuvio.tv/covers).
   ...
   - Je bent ook van harte welkom om je eigen afbeeldingen of GIF's te uploaden naar de community-collecties van Nuvio, zodat anderen ze kunnen gebruiken.

[Terug naar boven](#collecties)

### Je eerste collectie maken

Je kunt een collectie maken in de app of op de Nuvio-website — de website wordt aanbevolen.

1. Zorg er na het inloggen voor dat je op het tabblad **Account** bent.
2. Selecteer **Collections** (Collecties) in de linkerzijbalk.
3. Selecteer **Create Collection** (Collectie maken).
4. Er wordt gevraagd om een startsjabloon te kiezen — selecteer **Continue** (Doorgaan) om dit voor nu over te slaan.
5. Configureer de algemene instellingen van de collectie:
   - **Collection ID**: laat dit ongewijzigd.
   - **Title**: de naam die voor de collectie wordt weergegeven.
   ...
   - **Show All Tab**: voegt een extra tabblad toe dat alle submappen in deze collectie combineert.
   - **Pin to Top**: verplaatst de collectie naar boven, vóór andere collecties.
   - **Enable Focus Glow**: voegt een oplichtend effect toe bij het aanwijzen van de collectie.
6. Schakel over naar het tabblad **Folders** (Mappen) om de inhoud van de collectie te configureren — dit wordt hieronder in detail besproken.

[Terug naar boven](#collecties)

### Het tabblad Folders

Op het tabblad Folders definieer je de submappen ("blokken") waaruit je collectie bestaat:

- **Outline** (linkerzijbalk): voeg één item toe per blok dat je op je startscherm wilt tonen. Een collectie "Franchises" kan bijvoorbeeld afzonderlijke items hebben voor The Matrix, The Lord of the Rings en Star Wars.
- **Folder ID**: laat dit ongewijzigd.
- **Folder Title**: de naam die wordt getoond voor het tabblad of de rij van deze map. In een collectie "Actie" kunnen mappen bijvoorbeeld de titel "Actiefilms 2020" en "Actiefilms 2010" hebben.
...
Kies de add-on die de gewenste catalogus bevat (bijv. AIOMetadata) en selecteer vervolgens de specifieke catalogus die je met die add-on hebt gemaakt.
...
Deze instellingen bepalen de basisweergave en volgorde van je collectie binnen Nuvio.

- **Type**: het mediaformaat voor de lijst — ofwel **Movie** (Film) of **Series** (Serie).
- **Name**: de aangepaste titel voor deze collectie, weergegeven zoals ingevoerd (bijv. "Top Sci-Fi" of "Mijn volglijst").
- **Sort By**: (Sorteren op)
...
- **ID Field**: een numerieke of alfanumerieke code, te vinden in de URL van de betreffende lijst, collectie, bedrijf of trefwoord op TMDB of Trakt. Bijvoorbeeld: in `themoviedb.org/collection/1248` is de ID `1248`.

**Unieke bronnen**

Een paar bronnen gebruiken andere invoervelden in plaats van een standaard-ID:

- **TMDB Discover**: selecteer in plaats van een ID een of meer genres (bijv. Action, Horror, Science Fiction) uit een dropdown-menu. Dit bouwt dynamisch een collectie op van media die overeenkomen met die categorieën uit de database van TMDB.
...
Wanneer je een nieuwe collectie met Trakt als bron maakt, configureer dan eerst deze basisopties:

- **Type**: het mediaformaat voor de rij — ofwel **Movie** (Film) of **Series** (Serie).
- **Name**: het aangepaste label voor deze collectie, getoond op je Nuvio-startscherm (bijv. "Mijn Trakt-volglijst" of "Populaire series").
- **Sort By**: (Sorteren op)
...
>
> Het gebruik van een `app.trakt.tv`-link zorgt ervoor dat de collectie niet kan synchroniseren en leeg blijft.
